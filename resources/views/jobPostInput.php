...
class JobPostClass extends Controller
{
	public function JobPost()
	{
		...
		if($_SESSION['is_login'] == true){
			$temp = DB::select(DB::raw("select name from userinfo where userPK=".$_SESSION['userPK']));
			foreach($temp as $item){
				$GLOBALS['recruiterName'] = $item->name;
			}
		}
	}
}
...
if($_GET['inputFormType']=="new"){
	...
	<div id="projectInputForm">';
		...
		<div id="jobInfo" class="inputForm">
			<div class="inputGroup"><label class="label inputLabel">모집분야/포지션<span class="required">&nbsp;(필수사항)</span></label><textarea id="position" class="inputBox"></textarea><br></div>
			<div class="inputGroup"><label class="label inputLabel">담당업무 설명</label><textarea id="jobDesc" class="inputBox"></textarea><br></div>
			<div class="inputGroup"><label class="label inputLabel">필요 기술</label><textarea id="skill" class="inputBox"></textarea><br></div>
		...
		<div id="qualificationInfo" class="inputForm">
			...
			<label class="label inputLabel">경력</label><input id="expNone" type="radio" name="experience" value="none" checked>무관<input id="expPlus" type="radio" name="experience" value=""><input id="experiencePlus" type="text" name="experience" placeholder="ex.영화 제작 참여, 3년 이상, …"><br>
		</div>
	...
	<div id="projectInputForm">';
		// <div class="inputForm">
			// echo'<label class="label inputLabel">구인/구직</label>';
